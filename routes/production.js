...
router.post("/:id/approve", requireAuth, requireRole("admin"), requirePermission("production.approve"), async (req, res) => {
  try {
    ...
    if (existing.rows[0].status !== "DRAFT") return res.status(400).json({ error: "أمر التصنيع ده مش في حالة قابلة للاعتماد", code: "INVALID_STATE" });
    const result = await pool.query(
...
router.post("/:id/start", requireAuth, requirePermission("production.create"), async (req, res) => {
  ...
  try {
    ...
    if (order.status !== "APPROVED") { await client.query("ROLLBACK"); return res.status(400).json({ error: "أمر التصنيع ده مش في حالة قابلة للبدء", code: "INVALID_STATE" }); }
...
router.post("/:id/complete", requireAuth, requirePermission("production.complete"), async (req, res) => {
  ...
  try {
    ...
    if (existing.rows.length === 0) { await client.query("ROLLBACK"); return res.status(404).json({ error: "أمر التصنيع مش موجود" }); }
    const order = existing.rows[0];
    if (order.status !== "IN_PROGRESS") { await client.query("ROLLBACK"); return res.status(400).json({ error: "أمر التصنيع ده مش في حالة قابلة للإكمال", code: "INVALID_STATE" }); }
    if (!assertOwnBranch(req.user, order.branch_id)) {
      await client.query("ROLLBACK");
...
router.post("/:id/cancel", requireAuth, requirePermission("production.cancel"), async (req, res) => {
  ...
  try {
    ...
    if (order.status === "COMPLETED" || order.status === "CANCELLED") {
      await client.query("ROLLBACK");
      return res.status(400).json({ error: "أمر التصنيع ده اكتمل أو اتلغى بالفعل", code: "INVALID_STATE" });
    }
    if (!assertOwnBranch(req.user, order.branch_id)) {
      await client.query("ROLLBACK");
