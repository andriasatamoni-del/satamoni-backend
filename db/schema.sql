...
CREATE TABLE branches (
  ...
  -- تكامل طلبات (TAL-5): معرّف الفرع/المتجر عند Talabat - بيتحط يدويًا من الأدمن مرة واحدة لكل فرع
  -- (إدارة الربط، مش من أي API خارجي) عشان webhook الأوردرات يعرف يحوّل معرّف متجر Talabat لفرع Stamoni
  -- حقيقي. NULL يعني الفرع ده لسه مش مربوط بأي متجر Talabat.
  talabat_branch_id TEXT UNIQUE,
...
-- المرحلة 8.17: بطاقات الصفحة الرئيسية (public/index.html) كانت ثابتة في كود الـHTML نفسه (اسم/وصف/
-- ترتيب كل بطاقة مكتوب صراحة) - المالك عايز يقدر يغيّر اسم/ترتيب أي بطاقة من لوحة الأدمن من غير ما
-- يحتاج تعديل كود. tile_key = اسم ملف الشاشة نفسه (زي href بالظبط) - ثابت ومش بيتغيّر حتى لو الاسم
-- المعروض (title) اتغيّر، عشان أي مرجع مستقبلي ليه (زي دمج شاشات) يفضل مستقر
...
INSERT INTO home_tiles (tile_key, href, icon, title, description, display_order) VALUES
  ...
  ('menu', 'satamoni-menu.html', '📋', 'المنيو والإعدادات (أدمن)', 'إضافة الأصناف والأسعار، مناطق التوصيل، طرق الدفع، وإعدادات الطباعة', 110),
  ('procurement-hub', 'satamoni-procurement-hub.html', '📦', 'المشتريات والتصنيع', 'طلبيات الفرع، طلبيات السنتر كيتشن، المشتريات، التصنيع والتعبئة، وتخطيط التصنيع - كل واحدة في تاب منفصل جوة شاشة واحدة', 130),
...
CREATE TABLE menu_item_modifiers (
  ...
  is_active     BOOLEAN DEFAULT TRUE,
  -- المرحلة 8.9: لو المرفق ده من نوع "بدون" (بدون طماطم/بدون سدق..)، ممكن يتربط اختياريًا بمكوّن واحد من
  -- وصفة نفس الصنف (menu_item_variant_ingredients) - لما يتختار في طلب، المكوّن ده بيتشال من حساب
  -- الاستهلاك والتكلفة لسطر الطلب ده بس (الوصفة الأصلية فضلت زي ما هي، الاستثناء وقت البيع بس).
  -- بدون REFERENCES هنا عمدًا - inventory_items لسه متعرّفتش في الملف دلوقتي، الـFK متضاف بـALTER TABLE
  -- تحت بعد ما الجدول يتعرّف (نفس نمط order_items.recipe_version_id بالظبط)
...
CREATE TABLE orders (
  ...
  -- المرحلة 8.16: رقم أوردر طلبات (source='talabat' بس عمليًا) - بيتسجل وقت إدخال الطلب من الكاشير
  -- لسهولة المراجعة بعد كده (مطابقة مع كشف حساب تطبيق طلبات الشهري). مش FK ولا مقيّد بصيغة معينة -
  -- مجرد رقم/كود بيكتبه الكاشير زي ما هو ظاهر في تطبيق طلبات
  talabat_order_id      TEXT,
  -- المرحلة 8.16: أوردرات طلبات غالبًا كلها "آجل" (بتتحصّل من طلبات في تسوية دورية، حساب 1350) - بس
  -- أحيانًا جزء من قيمة الطلب بيتحصّل كاش في الفرع لحظة التسليم لطيار طلبات (خصم شركة طلبات نفسها بيغطي
...
CREATE TABLE order_item_modifiers (
  ...
  -- نسخة (snapshot) من menu_item_modifiers.excluded_ingredient_item_id وقت البيع بالظبط - لو ربط
...
-- المرحلة 8.9: الـFK المؤجّل بتاع menu_item_modifiers/order_item_modifiers.excluded_ingredient_item_id
-- فوق (inventory_items لسه ما كانتش متعرّفة وقتها)
ALTER TABLE menu_item_modifiers ADD CONSTRAINT fk_menu_item_modifiers_excluded_ingredient
