...
## استيراد ملفات (CSV/Excel) + مطابقة تلقائية - Phase 2

`db/payment-reconciliation-import.js` - قراءة ملفات CSV/Excel لكشوف طلبات/فيزا/إنستاباي/أورانج كاش
بدل إدخال كل سطر يدوي. **الاستيراد موضعي بالكامل عمدًا**: مفيش ملف حقيقي من أي مزوّد (طلبات/فيزا/
إنستاباي/أورانج كاش) اتاح وقت الكتابة، فبدل ما نخمّن أسماء أعمدة ثابتة ونخاطر نقرا عمود غلط بصمت (خطر
حقيقي في أداة كشف احتيال)، المحاسب بيشوف عيّنة من صفوف الملف الخام (`POST .../import/preview`) ويحدد
بنفسه أي عمود التاريخ/المبلغ/المرجع بعينه قبل ما يستورد فعليًا (`POST .../import/commit`). لما ملف
حقيقي يتاح، ممكن يتضاف تخمين تلقائي لأسماء الأعمدة فوق الآلية دي (تحسين، مش استبدال).

- كل سطور نفس الاستيراد بتتوسم بـ`import_batch_id` مشترك - لو عمود اتحدد غلط، `DELETE
  .../import-batches/:batchId` بيلغي الدفعة كلها مرة واحدة (بس لو لسه كل سطورها `UNMATCHED` - سطر
  اتطابق بالفعل لازم يتراجع يدويًا، مش أوتوماتيك).
- **المطابقة التلقائية (`autoMatchChannelRecords`) مقصورة على إنستاباي/أورانج كاش بس** - طلبات (كاش)
  وفيزا (تسوية) فحوصهم أصلًا مقارنة إجمالي فترة مقابل إجمالي فترة (`findTalabatCashDiscrepancies`/
  `findVisaSettlementDiscrepancy`)، مفيش مفهوم "سطر داخلي = سطر خارجي واحد لواحد" هناك من الأساس
  يتطابق. استيراد ملف لطلبات/فيزا بيوفّر وقت الكتابة اليدوية بس، من غير خطوة مطابقة بعده.
- مطابقة "مؤكدة" بس (`unique mutual match` - نفس فلسفة "مايخمّنش" في كل الوحدة): سطر كشف له دفعة
  واحدة مرشحة ضمن سماحية المبلغ (±1 ج.م) والتاريخ (±3 أيام)، والدفعة دي نفسها مالهاش مرشح سطر تاني -
  أي غموض (أكتر من مرشح لأي طرف) بيتسيب `UNMATCHED` عمدًا. `POST .../match-auto` بيعيد تشغيلها يدويًا
  (مفيد لو دفعات جديدة اتقفلت بعد ما الملف اتستورد أصلًا).

## حدود معروفة (صراحة، مش مخفية)

1. **الاستيراد موضعي (اختيار عمود يدوي)، مش تخمين أسماء أعمدة ثابتة** - لحد ما ملف حقيقي من مزوّد
   فعلي يتاح ويتراجع الشكل بتاعه، ده أضمن حل، بس معناه المحاسب لازم يختار الأعمدة كل مرة (مش memorized
   تلقائيًا لكل مصدر - تحسين مستقبلي معقول لو نفس الشكل بيتكرر شهريًا).
